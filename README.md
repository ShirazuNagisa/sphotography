...
### 👤 站点信息
- **个人信息展示** —— 可选「边栏一行」（头像 + 昵称，位于边栏收起按钮上方，默认）或「右下角卡片」（含简介、一言），也可完全关闭
- **一言格言** —— 可选的每次刷新随机格言（卡片方式下展示）
- **自定义页脚** —— 可选的毛玻璃页脚区块

### 🧪 实验性功能
- **AI 模块（自带 Key）** —— 在后台填入自己的 OpenAI 兼容 API，开启后可在文章编辑页：
  - **文章补全** —— 依据正文已有文字与图片补全/续写，关键词可留空；可调**感情风格**（更理性/普通/更感性）与**文案长度**（更长/中等/更短）；先预览再插入正文末尾
  - **润色** —— 分析当前正文与图片，保持篇幅、不增删内容，仅改善表达；可调风格；预览后一键替换正文（Ctrl+Z 可撤销）
  - **AI 自动标签** —— 依据正文建议中文标签，点击采用（仅写入原生文章标签，不影响地区标签）
  - 输出为 **WordPress 原生 HTML**（自动解析为区块），不使用 Markdown
- **单 / 双模型** —— 单模型（可多模态，或关闭图片分析仅用文字）；或双模型分别配置**识图模型 + 文案模型**，先识图生成描述再写作。图片经缩放后加密内联发送、从服务器端读取，本地站点亦可用
- **安全存储** —— 两套 API Key 均经 AES-256 加密后存于数据库、仅服务器端调用、从不回显到前端；总开关默认关闭，使用前附风险提示与连接测试
- **编辑器适配** —— 深色后台下，编辑器内 AI 面板强制浅色，避免与无深色模式的编辑器不协调
- **可扩展接口** —— 内置 `sphotography_ai_chat()` / `sphotography_ai_generate()` 接口，后续 AI 功能均可复用
...
│   └── ai-metabox.js            # 文章编辑页 AI 面板交互（实验性）
├── admin/
...
│   └── ai.php                   # 实验性 AI 接口（sphotography_ai_chat）+ 加密存储 + 编辑器面板
