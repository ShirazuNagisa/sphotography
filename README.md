...
### 🧪 实验性功能
- **AI 模块（自带 Key）** —— 在后台填入自己的 OpenAI 兼容 API（Base URL + Key + 模型），开启后可在文章编辑页：
  - **按关键词扩写正文** —— 输入关键词/提纲，生成后先预览、确认再插入正文末尾
  - **AI 自动标签** —— 依据正文建议中文标签，点击采用（仅写入原生文章标签，不影响地区标签）
- **安全存储** —— API Key 经 AES-256 加密后存于数据库、仅服务器端调用、从不回显到前端；总开关默认关闭，使用前附风险提示与连接测试
- **可扩展接口** —— 内置统一的 `sphotography_ai_chat()` 接口，后续 AI 功能均可复用
